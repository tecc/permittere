 ...
 */
import { getParents, hasParent } from "permittere/permissions";
import { prefer } from "permittere/strategies/conflict";
 ...
 */
export function explicitParentsOverChildren(fallback = parentsOtherwiseChildren()) {
 ...
 * @see ResolutionStrategy
 */
export function explicitChildrenOverParents(fallback = direct()) {
    const strategy = (child, map, has, resolve, conflict) => {
        const preferExplicit = prefer(true, conflict);
